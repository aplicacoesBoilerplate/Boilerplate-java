CREATE TABLE push_subscription (
   id_subscription BIGINT AUTO_INCREMENT COMMENT 'Unique push subscription identifier',
   id_user BIGINT NOT NULL COMMENT 'Reference to the user associated with this push subscription',
   endpoint VARCHAR(512) NOT NULL UNIQUE COMMENT 'Unique browser push service endpoint used to deliver notifications',
   p256dh VARCHAR(255) NOT NULL COMMENT 'Client public encryption key used for Web Push payload encryption',
   auth VARCHAR(255) NOT NULL COMMENT 'Authentication secret used for secure Web Push message encryption',
