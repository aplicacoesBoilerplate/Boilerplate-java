...
public class CAutorizacaoRbacManager implements AuthorizationManager<RequestAuthorizationContext> {
    ...
    public AuthorizationDecision authorize(Supplier<? extends Authentication> pAuthentication, RequestAuthorizationContext pContext) {
        ...
        String permissao = resolverPermissao(pContext.getRequest().getMethod(), pContext.getRequest().getServletPath());
        return new AuthorizationDecision(permissao != null && principal.contexto().permissions().contains(permissao));
    }

    private String resolverPermissao(String pMetodo, String pCaminho) {
        return REGRAS_PERMISSAO.stream()
                .filter(pRegra -> pRegra.metodo().equalsIgnoreCase(pMetodo)
                        && pathMatcher.match(pRegra.caminho(), pCaminho))
