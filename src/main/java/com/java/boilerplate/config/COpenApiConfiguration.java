...
public class COpenApiConfiguration {
    ...
    private ApiResponse criarResposta(String pDescricao) {
        return new ApiResponse()
                ...
                .content(new Content()
                        .addMediaType(CONTENT_TYPE_ACTUATOR, new MediaType().schema(criarSchemaHealthCheck())));
    }
    ...
    private Schema<?> criarSchemaHealthCheck() {
        ...
        schema.addProperty(
                "groups", new ArraySchema().items(new StringSchema()).example(List.of("liveness", "readiness")));
        return schema;
    }
}
